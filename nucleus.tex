% REF:
%Sect 4.4:
%In this section you discuss the silicate emission and the apparently overly strong 11.2 PAH emission. If this part is maintained it should probably be an separate section rather than a subsection.
...
% REF:
%Fig 13:
%It is difficult to get a good feeling for what we are looking at. I had to get HST images and run Aladin to understand what might be what in this region. It would help the reader a lot if you would make the first two panels much larger and omit the second and annotate the maps much more. Where is the double nucleus? Where is the cluster of A-stars? Is there a position for an X-ray source?
...
components are separated by 0\farcs5, which is well below the IRS spatial resolution. The contours in the right panel indicate
show the distribution of 3.6~$\mu$m luminosity in the same region, indicating the orientation of the M31 bulge.
\label{nuc11}
\end{figure}
...
% REF:
%The last paragraph of S4.4 is too speculative and should be removed, for the following reasons:
%
%1) The spectral shapes of the galaxies that were used in Spinoglio et al 1995 to calibrate this relation, has nothing to do with the shape of the nuclear spectrum you analyse here.
%
%I have published the spectrum of a galaxy whose emission is completely dominated by AGN activity (Hony et al 2011), much more like your central region and I calculate that the L_12/L_bol is about 30 per cent.
%
%2) Since you do not discuss the origin of the continuum, it is hard to understand how the 12um flux maps onto integrated galaxies luminosity. This cannot be the same mapping if the continuum is stellar in origin or AGN-torus.
%
%3) To use this equation you have to convolve your spectrum with the IRAS 12 filter to obtain a IRAS 12 flux density.
%
%4) There are Herschel data that can be used to do a direct integration rather than relying on this formula.
%
%5) Given the above caveats, the conclusion that the nucleus is IR to X-ray bright is pure speculation which is used to further speculate that the nucleus was brighter in the past.
