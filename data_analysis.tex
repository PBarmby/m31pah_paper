...
All of the main PAH features, including the 6.2, 7.7, 8.6 and 11.2~$\mu$m bands, 
...
suppression of the common 6--8~$\mu$m PAH bands and a strong 11.2~$\mu$m PAH band.
The 11.2~$\mu$m band profile was found to vary from region to region.  
...
Indeed all regions, except the nucleus, show a normal mid-infrared spectrum similar to other nearby star-forming galaxies. 
...
We obtained these newly-processed ISOCAM spectra from three regions in our IRS sample (see Section~\ref{sect:iso_data}) 
...
our modifications of PAHFIT to analyze this spectrum are discussed in Section~\ref{sect:nucleus}.
...
\hline
    {Region }&{5.7 $\mu$m  }&{6.2 $\mu$m  }&{7.7 $\mu$m$^b$  }&{8.3 $\mu$m  }&{8.6 $\mu$m  }&{10.7 $\mu$m  }&{11.2 $\mu$m$^b$  }&{12.0 $\mu$m  }&{12.7 $\mu$m$^b$  }&{17.0 $\mu$m$^b$  } \\
 \hline
  \input{tab3}
 \hline
 \label{PAHlinetable}
...
{$^a$Units are $\mu$m.\\
$^b$PAH complexes, as described in text.\\
$^c$Continuum for Regions 3 and 9 is very weak.  Equivalent widths are highly uncertain and not considered in the analysis (see Section~\ref{sect:data_analysis}).}
}
\end{minipage}
\end{table*}
...
For easier comparison with previous work, the tables give measurements for the 7.7, 11.2, 12.7 and 17.0~$\mu$m PAH complexes
