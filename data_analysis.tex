...
%SPW: In a similar manner, Sec 3.1 par 2 makes one expect a direct comparison between the old and new ISO spectra. ÊI don't think one is needed, given the clear reason for thinking the old spectra are bad, so I suggest just deleting the text that mentions such a comparison. ÊAgain alternatives are possible.
...
%SPW:In Fig 9, two points (if plotted correctly) are inconsistent with the starbursts. ÊThe text implies there are no inconsistencies. ÊPlease check the text and data.
\begin{figure}
\centering
